...
async function main() {
    ...
    // Regenerate root index.html as landing page
    if (generatedNiches.length > 0) {
        console.log('\n📄 Regenerating root landing page...');
        generateLandingPage(generatedNiches);
        console.log('✅ Root landing page created');
    }
...
async function generateSiteForNiche(niche) {
    ...
    // Generate main index.html using new template
    console.log('📄 Creating index.html using new template...');
    const indexHTML = generateNewTemplateHTML(niche, slug, products);
...
/**
 * Generate table rows for comparison table
 * @param {Array} products - Products array
...
function generateTableRows(products) {
    return products.map((product, index) => {
        const rank = index + 1;
        const keyFeature = product.features && product.features[0] 
            ? truncate(product.features[0], 50) 
            : 'See details';
        const affiliateLink = generateAffiliateLink(product);
        
        return `  <tr>
    <td>${rank}</td>
    <td>${escapeHtml(truncate(product.title, 40))}</td>
    <td>${product.rating} ⭐ (${product.reviews})</td>
    <td>${escapeHtml(keyFeature)}</td>
    <td><a href="${affiliateLink}" target="_blank" rel="nofollow noopener">View on Amazon</a></td>
  </tr>`;
    }).join('\n');
}

/**
 * Generate product blocks for full reviews
 * @param {Array} products - Products array
 * @returns {string} Product blocks HTML
 */
function generateProductBlocks(products) {
    return products.map((product, index) => {
        const rank = index + 1;
        const affiliateLink = generateAffiliateLink(product);
        
        let block = `<div class="product">
  <h3>#${rank} - ${escapeHtml(product.title)}</h3>
  <img src="${product.image}" alt="${escapeHtml(product.title)}" />
  <p><strong>Rating:</strong> ${product.rating} ⭐ (${product.reviews} reviews)</p>
  <p><strong>Price:</strong> ${product.price}</p>
  <p>${escapeHtml(truncate(product.description, 300))}</p>`;
  ...
  <ul>`;
        ...
        block += `\n  <a href="${affiliateLink}" class="btn" target="_blank" rel="nofollow noopener">Check Price on Amazon</a>
</div>`;
...
/**
 * Generate ItemList JSON-LD schema with all products
 * @param {string} niche - Niche name
 * @param {string} slug - URL slug
 * @param {Array} products - Products array
 * @returns {string} JSON-LD schema as string
 */
function generateItemListSchema(niche, slug, products) {
    const schema = {
        "@context": "https://schema.org",
        "@type": "ItemList",
        "itemListElement": products.map((product, index) => ({
...
/**
 * Generate index HTML using new template format
 * @param {string} niche - Niche name
 * @param {string} slug - URL slug
 * @param {Array} products - Products array
 * @returns {string} Complete HTML
 */
function generateNewTemplateHTML(niche, slug, products) {
    // Read the new template from templates directory
    const newTemplate = fs.readFileSync(path.join(CONFIG.TEMPLATES_DIR, 'new-template.html'), 'utf-8');
    ...
    const siteUrl = `${CONFIG.BASE_URL}/${slug}/`;
    const ogImage = products[0]?.image || 'https://via.placeholder.com/1200x630';
    
    const tableRows = generateTableRows(products);
    const productBlocks = generateProductBlocks(products);
    const schemaMarkup = generateItemListSchema(niche, slug, products);
    
    let html = newTemplate;
    
    // Replace all placeholders
    html = html.replace(/{{NICHE}}/g, niche);
    html = html.replace(/{{YEAR}}/g, year.toString());
    ...
    html = html.replace(/{{PRODUCT_BLOCKS}}/g, productBlocks);
    
    // Inject schema markup before </body>
    html = html.replace('</body>', `${schemaMarkup}\n</body>`);
    
    return html;
}

/**
 * Generate root landing page listing all niche sites
 * @param {Array} generatedNiches - Array of generated niche objects
 */
function generateLandingPage(generatedNiches) {
    const year = new Date().getFullYear();
...
<body>
  ...
  <ul>
${generatedNiches.map(({ niche, slug }) => `    <li>
      <a href="${slug}/index.html">Top 10 ${niche} (${year})</a>
      <div class="year">Updated for ${year}</div>
    </li>`).join('\n')}
  </ul>
  <div class="footer">
    ...
    fs.writeFileSync(path.join(CONFIG.OUTPUT_DIR, 'index.html'), landingHTML);
}
